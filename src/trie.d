...
template RawRadixTree(Value = void)
{
    ...
    */
    ...
    static if (isValue)
    {
        ...
        /// ditto
        inout(Value*) containsAt(inout Node curr, UKey key) @trusted
        {
            assert(key.length);
            // debug if (willFail) { dbg("key:", key); }
    ...
    else
    {
        ...
        /// ditto
        bool containsAt(Node curr, UKey key) @trusted
        {
            assert(key.length);
            // debug if (willFail) { dbg("key:", key); }
            import std.algorithm.searching : skipOver;
    ...
    inout(Node) prefixAt(inout Node curr, UKey keyPrefix, out UKey keyPrefixRest) @trusted pure nothrow @nogc
    {
        import std.algorithm : startsWith;
        final switch (curr.typeIx) with (Node.Ix)
        {
    ...
    inout(Node) matchCommonPrefixAt(inout Node curr, UKey key, out UKey keyRest) @trusted pure nothrow @nogc
    {
        // dbg(curr.typeIx);
        import std.algorithm : startsWith;
        final switch (curr.typeIx) with (Node.Ix)
