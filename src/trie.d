...
struct RawRadixTree(Value = void)
{
    ...
    @safe pure nothrow /* TODO @nogc */
    {
        ...
        static if (!hasValue)
        {
            Node insertNew(UKey key, out Node insertionNode)
            {
                ...
                default:
                    ...
                    {
                        return Node(insertNewBranch(key, insertionNode));
                    }
                }
            }
        }
        ...
        pragma(inline) Node insertAt(Node curr, UKey key, out Node insertionNode)
        {
            ...
            if (!curr)          // if no existing `Node` to insert at
            {
                static if (hasValue)
                {
                    auto next = Node(insertNewBranch(key, insertionNode));
                }
                else
                {
            ...
            else
            {
                ...
                case ix_DenseBranchPtr:
                    return Node(insertAtBranchAbovePrefix(Branch(curr.as!(DenseBranch*)), key, insertionNode));
                }
            }
        }
        ...
        Node insertAtLeaf(Leaf curr, UKey key, out Node insertionNode)
        {
            ...
            if (key.length == 1)
            {
                return Node(insertIxAtLeaftoLeaf(curr, key[0], insertionNode));
            }
            else
            {
                assert(key.length >= 2);
        ...
        static if (!hasValue)
        {
            Node insertAt(OneLeafMax7 curr, UKey key, out Node insertionNode)
            {
                ...
                return Node(insertAtBranchAbovePrefix(expand(curr), key, insertionNode));
            }
    ...
    */
    Branch expand(SparseBranch* curr, size_t capacityIncrement = 1)
    {
        ...
        else if (curr.subLength + capacityIncrement <= curr.maxCapacity) // if we can expand to curr
        {
            ...
            import std.algorithm : min;
            const nextCapacity = min(nextPow2(cast(uint)curr.subCapacity + capacityIncrement - 1),
                                     DefaultBranch.maxCapacity);
            assert(nextCapacity >= curr.subCapacity + capacityIncrement);
            next = constructVariableLength!(typeof(curr))(nextCapacity, curr);
        }
        else
        {
    ...
    */
    Leaf expand(SparseLeaf1!Value* curr, size_t capacityIncrement = 1)
    {
        ...
        else if (curr.length + capacityIncrement <= curr.maxCapacity) // if we can expand to curr
        {
            ...
            import std.math : nextPow2;
            import std.algorithm : min;
            const nextCapacity = min(nextPow2(cast(uint)curr.capacity + capacityIncrement - 1),
                                     curr.maxCapacity);
            assert(nextCapacity >= curr.capacity + capacityIncrement);
            static if (hasValue)
            {
                next = constructVariableLength!(typeof(curr))(nextCapacity, curr.ixs, curr.values);
            }
            else
            {
                next = constructVariableLength!(typeof(curr))(nextCapacity, curr.ixs);
            }
        }
        else
        {
    ...
    */
            ...
            hasVariableLength!NodeType)
    {
        ...
        capacity = max(NodeType.minCapacity, capacity); // limit capacity
